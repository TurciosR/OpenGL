...
#include <unistd.h>
#include <math.h>
#include "funciones.cpp"//llamando las funciones
#define MUS_PATH "audio.wav"//audio
...
float x=0.0f,z=5.0f,zz=0.0,Az=0.0, y=0,Ty=0.0;
int P = 0;
int frameNumber, pasos;

//funcion para las texturas del texto
void text(float P)
{
  char text[32];
  if(P == -1.0)
    {
        sprintf(text, "Perdiste");
        frameNumber=0;
      }

  else if(P== -2.0)
    {
        sprintf(text, "Ganaste");
      }
  else
      {
        sprintf(text, "Puntuacion: %.0f", P);
      }
      //definiendo color, tipo y tamaño de letra
      glColor3f(1, 1, 1);
...
void display(void)
{
  ...
  glLoadIdentity();
  gluLookAt(0.0f, 1.0f, 500.0f, 0.0, 1.0f, 490,  0.0f, 1.0f, 0.0f);
  glTranslatef(0.0f , 0.0f, Az+495+(-zz/-vzR));
  arbol();
  cesped();
  camino();
  obstaculos();
  glPushMatrix();
  float vz = -5.0-(-zz/-vzR)-0.75;
  double Rvz = round(vz);
  glTranslatef(1.0, 2.5f, -5.0-(-zz/-vzR));
  text(frameNumber);
  glPushMatrix();
  glTranslatef(x-1.0 , -1.9f, Az+-5.0-(-zz/-vzR)-(-5.0-(-zz/-vzR)));
...
void my_audio_callback(void *userdata, Uint8 *stream, int len)
{
  ...
  // Simplemente copie desde un buffer en el otro
